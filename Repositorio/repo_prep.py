    ...
    def _generate_addons_files(self):
        ...
        for addon in addons:
            try:
                ...
                if is_addon_dir(addon):
                    ...
                    zip_file = [x for x in list(
                        os.listdir(addon)) if x[-4:] == ".zip"]

                    if zip_file and metadata is not None:
                        addon_zip = zipfile.ZipFile(
                            os.path.join(addon, zip_file[0]))
                        ...
                        try:
                            ...
                            print(
                                "Could not extract assets from %s because %s" % (_path, e, ))
        ...
        if found_an_addon:
            ...
            if self._generate_md5_file():
                ...
                raise Exception(
                    "Error updating addons xml and addons.xml.md5 files")
            ...
            if is_addon_dir(self.addon_path):
                ...
                if addon_zip_exists:
                    ...
                    if addon_xml_exists:
                        ...
                        print('Create compressed addon release for -- ' +
                              self.addon_name + '  v' + self.addon_version_number)
